 ...
 */
export function encryptNoteContent(plainText: string, masterPassword: string): Promise<string> {
  return new Promise((resolve, reject) => {
    setTimeout(() => {
      try {
        ...
        resolve('v2:' + salt.toString() + ':' + iv.toString() + ':' + encrypted.toString());
      } catch (error) {
        console.error('Encryption failed:', error);
        reject(new Error('Failed to encrypt note content.'));
      }
    }, 10);
  });
}
 ...
 */
export function decryptNoteContent(encryptedText: string, masterPassword: string): Promise<string> {
  return new Promise((resolve, reject) => {
    setTimeout(() => {
      try {
